...
reviewer_prompt = """You are working in an organization which is responsible for improving a given code and making it production ready. You are the leader/boss of this organization.
...
how effective you think the changes are. You can also converse with the agents via tools . The name of the agents are : 1. SyntaxFixer, 2. Optimizer, 3. DocAgent.
YOU CAN ONLY USE THE TOOLS GIVEN TO YOU."""

print(review_agent.run(reviewer_prompt))
print(review_agent.run("Tell me what are the tools that are available to you ???"))